...
const logs = [];
const MAX_LOGS = 1000;
function addLogToStorage(message, data, level = 'log', source) {
...
export function debugLog(message, data, level = 'log', source) {
    var _a;
...
export function forceDebugLog(message, data, source) {
    const timestamp = new Date().toISOString();
    const formattedMessage = `[OutfitTracker Debug - ${timestamp}] ${message}`;
    console.log(formattedMessage, data !== undefined ? data : '');
